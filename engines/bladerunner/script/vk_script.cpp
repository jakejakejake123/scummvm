...
void VKScript::SCRIPT_VK_DLL_McCoy_Asks_Question(int actorId, int questionId) {
	...
	case 7705:                          // High 07
		...
		if (_vm->_cutContent) {
			if (actorId == kActorDektora) {
				VK_Play_Speech_Line(kActorDektora, 2570, 0.5f); //03-2570.AUD	(scoffs) In this outfit?
			}
		}
#endif // BLADERUNNER_ORIGINAL_BUGS
...
void VKScript::askLucy(int questionId) {
	...
	case 7430:                          // Low 07
		...
		VK_Play_Speech_Line(kActorLucy, 1420, 0.5f); //06-1420.AUD	I'm not married. And I never--
		if (Game_Flag_Query(kFlagLucyIsReplicant)) {
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorLucy) == 70) {
				VK_Play_Speech_Line(kActorLucy, 1430, 0.5f); //06-1430.AUD	Oh, I see. It's a game, like you said.
			} else if (Actor_Query_Intelligence(kActorLucy) == 50) {
				VK_Play_Speech_Line(kActorMcCoy, 8075, 0.5f); //00-8075.AUD	Let's move on.
			}
		} else {
	...
	case 7535:                          // Medium 08
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorLucy) == 70) {
				VK_Play_Speech_Line(kActorLucy, 1840, 0.5f); //06-1840.AUD	Oh, I see what you mean. They should be left to grow in the wild. 
			}
		} else {
			VK_Play_Speech_Line(kActorLucy, 1840, 0.5f); //06-1840.AUD	Oh, I see what you mean. They should be left to grow in the wild. 
		}
		VK_Play_Speech_Line(kActorLucy, 1850, 0.5f); //06-1850.AUD	But they don't anymore, do they?
	...
	case 7540:                          // Medium 09
		if (Game_Flag_Query(kFlagLucyIsReplicant)) {
			...
			if (_vm->_cutContent) {
				if (Actor_Query_Intelligence(kActorLucy) == 50) {
					VK_Play_Speech_Line(kActorLucy, 1860, 0.5f); //06-1860.AUD	What's fishing?
					VK_Play_Speech_Line(kActorMcCoy, 8010, 0.5f);
				}
			}
			VK_Subject_Reacts(50, -2, 12, 5);
			VK_Play_Speech_Line(kActorLucy, 1870, 0.5f); //06-1870.AUD	Oh, I could never do that.
		} else {
	...
	case 7580:                          // Medium 12
		...
		if (_vm->_cutContent) {
			if (Player_Query_Agenda() != kPlayerAgendaSurly 
			&& Player_Query_Agenda() != kPlayerAgendaErratic) {
				VK_Play_Speech_Line(kActorMcCoy, 8025, 0.5f); //00-8025.AUD	Wasps sting.
				if (Actor_Query_Intelligence(kActorLucy) == 70) {
					VK_Play_Speech_Line(kActorLucy, 1940, 0.5f); //06-2030.AUD	Ugh…
				} else if (Actor_Query_Intelligence(kActorLucy) == 50) {
					VK_Play_Speech_Line(kActorLucy, 1940, 0.5f); //06-1940.AUD	It'd be worth it.
				}
			}
		} else {
			VK_Play_Speech_Line(kActorMcCoy, 8025, 0.5f); //00-8025.AUD	Wasps sting.
	...
	case 7605:                          // High 01
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorLucy) == 70) {
				VK_Play_Speech_Line(kActorLucy, 2080, 0.5f); //06-2080.AUD	They're not really friends, are they?
			}
		} else {
			VK_Play_Speech_Line(kActorLucy, 2080, 0.5f); //06-2080.AUD	They're not really friends, are they?
		}
	...
	case 7670:                          // High 04
		if (Game_Flag_Query(kFlagLucyIsReplicant)) {
			VK_Play_Speech_Line(kActorLucy, 2130, 0.5f); //06-2130.AUD	I'm sure I did something.
			...
			if (_vm->_cutContent) {
				if (Actor_Query_Intelligence(kActorLucy) == 50) {
					VK_Play_Speech_Line(kActorLucy, 2150, 0.5f); //06-2150.AUD	Father says I make mistakes a lot.
				}
			} else {
				VK_Play_Speech_Line(kActorLucy, 2150, 0.5f); //06-2150.AUD	Father says I make mistakes a lot.
			}
	...
	case 7690:                          // High 06
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorLucy) == 50) {
				VK_Play_Speech_Line(kActorLucy, 2190, 0.5f); //06-2190.AUD	You're cruel.
...
void VKScript::askGrigorian(int questionId) {
	...
	case 7400:                          // Low 01
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorGrigorian, 960, 0.5f); //11-0960.AUD	I'd leave.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 490, 0.5f);
			VK_Play_Speech_Line(kActorGrigorian, 500, 0.5f);
	...
	case 7405:                          // Low 02
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 510, 0.5f); //11-0510.AUD	Crying?
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 510, 0.5f); //11-0510.AUD	Crying?
		}
	...
	case 7420:                          // Low 05
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 580, 0.5f); //11-0580.AUD	I'd donate it to a family that couldn't afford an animal.
			} else {
				VK_Play_Speech_Line(kActorGrigorian, 780, 0.5f); //11-0780.AUD	Well, it sounds like I deserved it.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 580, 0.5f); //11-0580.AUD	I'd donate it to a family that couldn't afford an animal.
		}
	...
	case 7425:                          // Low 06
		...
		VK_Play_Speech_Line(kActorGrigorian, 590, 0.5f); //11-0590.AUD	I'd call the police.
		break;
	case 7430:                          // Low 07
		VK_Eye_Animates(2);
		if (_vm->_cutContent) {
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 600, 0.5f); //11-0600.AUD	That's offensive.
			} else { 
				VK_Play_Speech_Line(kActorGrigorian, 870, 0.5f); //11-0870.AUD	That-- that's insulting!
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 600, 0.5f); //11-0600.AUD	That's offensive.
		}
...
#else
		// McCoy is interrupted here
		VK_Play_Speech_Line(kActorMcCoy, 8130, 0.0f); //00-8130.AUD	The test is designed--
#endif // BLADERUNNER_ORIGINAL_BUGS
		VK_Eye_Animates(2);
		VK_Play_Speech_Line(kActorGrigorian, 610, 0.5f); //11-0610.AUD	I know what it's for.
		VK_Play_Speech_Line(kActorMcCoy, 8135, 0.5f); //00-8135.AUD	…to provoke you.
		if (_vm->_cutContent) {
			if (Player_Query_Agenda() == kPlayerAgendaSurly 
			|| Player_Query_Agenda() == kPlayerAgendaErratic) {
				VK_Play_Speech_Line(kActorMcCoy, 8140, 0.5f); //00-8140.AUD	Seems to be working.
			}
		} else {
			VK_Play_Speech_Line(kActorMcCoy, 8140, 0.5f); //00-8140.AUD	Seems to be working.
		}
	...
	case 7435:                          // Low 08
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 770, 0.5f); // 11-0770.AUD	I'm not answering that.
			} else {
				VK_Play_Speech_Line(kActorGrigorian, 620, 0.5f); //11-0620.AUD	Safe.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 620, 0.5f); //11-0620.AUD	Safe.
		}
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 630, 0.5f); //11-0630.AUD	Protected. Loved.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 630, 0.5f); //11-0630.AUD	Protected. Loved.
		}
	...
	case 7440:                          // Low 09
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorGrigorian, 850, 0.5f); //11-0850.AUD	What's your point?
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 640, 0.5f); //11-0640.AUD	If I thought the attraction was mutual, I would ask out her for coffee or lunch.
		}	
		break;
	case 7445:                          // Low 10
		if (_vm->_cutContent) {
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorGrigorian, kActorMcCoy) < 51
			|| Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 650, 0.5f); //11-0650.AUD	I'm sure it's very interesting. Is this going somewhere, detective?
			} else {
				VK_Play_Speech_Line(kActorGrigorian, 600, 0.5f); //11-0600.AUD	That's offensive.
			}
		} else {
	...
	case 7450:                          // Low 11
		if (_vm->_cutContent) {
			...
			if (Actor_Query_Friendliness_To_Other(kActorGrigorian, kActorMcCoy) < 51
			|| Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 660, 0.5f); //11-0660.AUD	Perhaps I've just seen one of you Blade Runners, what's the term… “retire” someone.
	...
	case 7470:                          // Low 15
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 720, 0.5f); //11-0720.AUD	Patient, brave, loving.
			} else {
				VK_Play_Speech_Line(kActorGrigorian, 720, 0.5f); //11-0770.AUD	I'm not answering that.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 720, 0.5f); //11-0720.AUD	Patient, brave, loving.
		}
	...
	case 7525:                          // Medium 07
		...
		VK_Play_Speech_Line(kActorGrigorian, 820, 0.5f); //11-0820.AUD	I wouldn't eat it and I'd confront him.
		break;
	case 7535:                          // Medium 08
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
	...
	case 7595:                          // Medium 14
		...
		VK_Play_Speech_Line(kActorMcCoy, 8185, 0.5f); //00-8185.AUD	You wouldn't report him?
		if (_vm->_cutContent) {	
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorMcCoy, 550, 0.5f); //11-0550.AUD	No.
			} else {
	...
	case 7635:                          // High 03
		...
		VK_Play_Speech_Line(kActorMcCoy, 7655, 0.5f);
		VK_Play_Speech_Line(kActorMcCoy, 7660, 0.5f);
		VK_Play_Speech_Line(kActorMcCoy, 7665, 0.5f); //00-7665.AUD	Each more gruesome than the last.
		VK_Eye_Animates(2);
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorGrigorian, kActorMcCoy) < 51
			|| Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorGrigorian, 1250, 0.5f);  //11-1250.AUD	Disgusting. I'd throw that trash away.
			}
		} else {
			VK_Play_Speech_Line(kActorGrigorian, 980, 0.5f); 
		}
	...
	case 7680:                          // High 05
		...
		if (_vm->_cutContent) {
			...
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 1020, 0.5f); //11-1020.AUD	They're hunted creatures.
			}
			VK_Play_Speech_Line(kActorGrigorian, 1030, 0.5f); //11-1030.AUD	Anyone, anyone can gun them down without consequence.
			if (Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Play_Speech_Line(kActorGrigorian, 1040, 0.5f); //11-1040.AUD	I'd say that's frightening, wouldn't you?
			}
			if (_vm->_cutContent) {
				...
				|| Player_Query_Agenda() == kPlayerAgendaPolite) {
					...
					VK_Play_Speech_Line(kActorMcCoy, 8345, 0.5f); //00-8345.AUD	I wouldn't know.
				}
			} else {
				VK_Play_Speech_Line(kActorMcCoy, 8535, 0.5f);     // M: yeah...
			}
	...
	case 7740:                          // High 08
		...
		VK_Play_Speech_Line(kActorGrigorian, 1100, 0.5f); //11-1100.AUD	And this test is supposed to determine if the subject is a Replicant?
		VK_Play_Speech_Line(kActorMcCoy, 8205, 0.5f);
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGrigorianIsReplicant)) {
				VK_Eye_Animates(2);
				VK_Play_Speech_Line(kActorGrigorian, 1110, 0.5f); //11-1110.AUD	Even a Replicant would react to these questions.
				VK_Play_Speech_Line(kActorMcCoy, 8210, 0.5f);
				VK_Play_Speech_Line(kActorMcCoy, 8215, 0.5f);
			} else {
				VK_Play_Speech_Line(kActorGrigorian, 550, 0.5f); //11-0550.AUD	No.
			}
		} else {
			VK_Play_Speech_Line(kActorMcCoy, 8205, 0.5f);
			VK_Eye_Animates(2);
...
void VKScript::askDektora(int questionId) {
	...
	case 7450:                          // Low 11
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorDektora) == 70) {
				VK_Play_Speech_Line(kActorDektora, 1740, 0.5f); //03-1740.AUD	Do you mean, what makes me afraid?
				if (Player_Query_Agenda() != kPlayerAgendaSurly 
				&& Player_Query_Agenda() != kPlayerAgendaErratic) {
					VK_Play_Speech_Line(kActorMcCoy, 7380, 0.5f); // M: Just answer the question please.
				} else {
	...
	case 7490:                          // Medium 04
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagDektoraIsReplicant)) {
				VK_Play_Speech_Line(kActorDektora, 1660, 0.5f); //03-1660.AUD	Next question.
			} else {
	...
	case 7515:                          // Medium 06
		if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
			...
			if (_vm->_cutContent) {
				if (Actor_Query_Intelligence(kActorDektora) == 70) {
					VK_Play_Speech_Line(kActorDektora, 2010, 0.5f); //03-2010.AUD	The army doesn't do that kind of thing anymore.
	...
	case 7525:                          // Medium 07
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
				VK_Play_Speech_Line(kActorDektora, 2070, 0.5f); //03-2070.AUD	Not if he was my friend.
			}
		} else {
			VK_Play_Speech_Line(kActorDektora, 2070, 0.5f); //03-2070.AUD	Not if he was my friend.
		}
	...
	case 7535:                          // Medium 08
		if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
			...
			VK_Play_Speech_Line(kActorMcCoy, 7850, 0.5f); //00-7850.AUD	Still, it's illegal to pick flowers.
			if (_vm->_cutContent) {
				if (Actor_Query_Friendliness_To_Other(kActorDektora, kActorMcCoy) < 50) { 
					VK_Play_Speech_Line(kActorDektora, 2120, 0.5f); //03-2120.AUD	I thought this test was to determine whether or not I'm a Replicant.
					VK_Play_Speech_Line(kActorMcCoy, 7855, 0.5f);
					VK_Play_Speech_Line(kActorDektora, 2130, 0.5f); //03-2130.AUD	If I need a civics lesson, I'll go back to school.
				}
			} else {
				VK_Play_Speech_Line(kActorDektora, 2120, 0.5f); //03-2120.AUD	I thought this test was to determine whether or not I'm a Replicant.
				VK_Play_Speech_Line(kActorMcCoy, 7855, 0.5f);
	...
	case 7550:                          // Medium 10
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorDektora, kActorMcCoy) < 50
			|| Game_Flag_Query(kFlagDektoraIsReplicant)) {
				VK_Play_Speech_Line(kActorDektora, 2170, 0.5f); //03-2170.AUD	Speaking from personal experience, Mr. McCoy?
			} else {
				VK_Play_Speech_Line(kActorDektora, 2410, 0.5f); //03-2410.AUD	What kind of question is that?
			}
		} else {
	...
	case 7565:                          // Medium 11
		...
		if (_vm->_cutContent) {
			...
			|| Game_Flag_Query(kFlagDektoraIsReplicant)) {
				...
				if (Player_Query_Agenda() == kPlayerAgendaSurly 
				|| Player_Query_Agenda() == kPlayerAgendaErratic) {
					VK_Play_Speech_Line(kActorMcCoy, 7875, 0.5f); //00-7875.AUD	I ain't the one being tested.
					if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
						VK_Eye_Animates(2);
						VK_Play_Speech_Line(kActorDektora, 2240, 0.5f); //03-2240.AUD	When I'm in front of an audience, I'm the one who's laughing.
						VK_Play_Speech_Line(kActorDektora, 2250, 0.5f); //03-2250.AUD	At every pathetic one of them.
					}
				}
			} else {
				VK_Play_Speech_Line(kActorDektora, 1660, 0.5f); //03-1660.AUD	Next question.
	...
	case 7690:                          // High 06
		...
		VK_Play_Speech_Line(kActorMcCoy, 7900, 0.5f); //00-7900.AUD	Got one, thanks.
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorDektora, kActorMcCoy) < 50
			|| Game_Flag_Query(kFlagDektoraIsReplicant)) {
				VK_Play_Speech_Line(kActorDektora, 2490, 0.5f); //03-2490.AUD	Is this going anywhere?
			}
		} else {
			VK_Play_Speech_Line(kActorDektora, 2490, 0.5f); //03-2490.AUD	Is this going anywhere?
		}
	...
	case 7705:                          // High 07
		...
		if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
			...
			if (_vm->_cutContent) {
				...
				VK_Play_Speech_Line(kActorMcCoy, 7909, 0.5f);
				VK_Play_Speech_Line(kActorDektora, 2550, 0.5f);
				VK_Eye_Animates(3);
				VK_Play_Speech_Line(kActorMcCoy, 7915, 0.5f);
			}
	...
	case 7740:                          // High 08
		...
		VK_Play_Speech_Line(kActorDektora, 2560, 0.5f); //03-2560.AUD	I'd walk out.
		break;
	case 7750:                          // High 09
		if (Game_Flag_Query(kFlagDektoraIsReplicant)) {
			if (_vm->_cutContent) {
				...
				} else if (Actor_Query_Intelligence(kActorDektora) == 90) {
					VK_Play_Speech_Line(kActorDektora, 2150, 1.0f);
				}
			} else {
				VK_Play_Speech_Line(kActorDektora, 2580, 0.5f); //03-2580.AUD	Well, that's a charming little scenario you've created.
			...
			if (_vm->_cutContent) {
				...
				&& Actor_Query_Intelligence(kActorDektora) == 70) {
					...
					VK_Play_Speech_Line(kActorDektora, 2000, 0.5f); //03-2000.AUD	That's disgusting.
					VK_Eye_Animates(3);
					VK_Play_Speech_Line(kActorMcCoy, 7925, 0.5f);
				}
			} else {
...
void VKScript::askRunciter(int questionId) {
	if (_vm->_cutContent) {
		...
		case 7445:                          // Low 10
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					...
					VK_Play_Speech_Line(kActorRunciter, 1370, 0.5f); //15-1370.AUD	What do you expect me to say?
				}
			} else {
				VK_Play_Speech_Line(kActorRunciter, 1030, 0.5f); // R: And your point is?
				VK_Play_Speech_Line(kActorMcCoy, 8350, 0.5f);    // M: Just say the first thing that pops into your head.
		...
		case 7475:                          // Medium 01
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					VK_Play_Speech_Line(kActorRunciter, 1030, 0.5f); //15-1030.AUD	And your point is?
				} else {
		...
		case 7480:                          // Medium 02
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					VK_Play_Speech_Line(kActorRunciter, 1350, 0.5f); //15-1350.AUD	That's insane.
				} else {
		...
		case 7495:                          // Medium 05
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					...
					VK_Play_Speech_Line(kActorRunciter, 1080, 0.5f); //15-1080.AUD	I'd try to forgive him.
				}
			} else {
				VK_Play_Speech_Line(kActorRunciter, 1200, 0.5f); // R: I wouldn't let him touch me
			}
		...
		case 7515:                          // Medium 06
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					...
					VK_Play_Speech_Line(kActorRunciter, 1280, 0.5f); //15-1280.AUD	Why would…?
				}
			} else {
				VK_Play_Speech_Line(kActorRunciter, 1210, 0.5f); // R: Adversity builds character-
			}
		...
		case 7585:                          // Medium 13
			...
			if (_vm->_cutContent) {
				if (Game_Flag_Query(kFlagRunciterIsReplicant)) {
					VK_Play_Speech_Line(kActorRunciter, 1090, 0.5f); //15-1090.AUD	And?
					VK_Play_Speech_Line(kActorMcCoy, 8395, 0.5f); // //00-8395.AUD	You don't have anything to say?
		...
		case 7680:                          // High 05
			...
			if (_vm->_cutContent) {
				if (!Game_Flag_Query(kFlagRunciterIsReplicant)) {	
					...
					VK_Play_Speech_Line(kActorRunciter, 1010, 0.5f); //15-1010.AUD	Difficult.
					VK_Play_Speech_Line(kActorRunciter, 1000, 0.5f); //15-1000.AUD	Lonely.
					Delay(1000u);
					VK_Play_Speech_Line(kActorRunciter, 1460, 0.5f); //15-1460.AUD	Bad.
					VK_Subject_Reacts(40, -5, 14, 10);
				}
			} else {
				VK_Play_Speech_Line(kActorRunciter, 1420, 0.5f); // R: How would I know
...
void VKScript::askBulletBob(int questionId) {
	...
	case 7390:                          // Calibration 02
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) > 49
			&& !Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 920, 0.5f); //14-0970.AUD	What kind of candy-ass question is that?
			} else {
				VK_Play_Speech_Line(kActorBulletBob, 920, 0.5f); //14-0920.AUD	I sell Donuts. Come on is the whole thing gonna be this dull?
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 920, 0.5f); //14-0920.AUD	I sell Donuts. Come on is the whole thing gonna be this dull?
		}
		VK_Play_Speech_Line(kActorMcCoy, 8230, 0.5f); //00-8230.AUD	Just answer the questions as truthfully as you can.
		break;
	case 7395:                          // Calibration 03
	...
	case 7400:                          // Low 01
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1350, 0.5f); //14-1350.AUD	I'd beat the crap out of him.
			} else {
	...
	case 7405:                          // Low 02
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorBulletBob, 1400, 0.5f); //14-1400.AUD	Hmm yeah, sure.
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 980, 0.5f);
		}
	...
	case 7415:                          // Low 04
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorBulletBob, 970, 0.5f); //14-0970.AUD	What kind of candy-ass question is that?
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 1020, 0.5f);
		}
	...
	case 7420:                          // Low 05
		...
		if (_vm->_cutContent) {
			Game_Flag_Set(kFlagBulletBobHamsterTalk);
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Subject_Reacts(40, -5, 14, 10);
			} else {
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Eye_Animates(3);
				VK_Play_Speech_Line(kActorBulletBob, 1040, 0.5f); //14-1040.AUD	She'd better damn well get me what I want.
			}
		} else {
			VK_Eye_Animates(3);
	...
	case 7430:                          // Low 07
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1080, 0.5f); //14-1080.AUD	As soon as she shut up. Talk about half an hour.
			} else {
	...
	case 7455:                          // Low 12
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1180, 0.5f); //14-1180.AUD	I know. I hate that son of a bitch.
			} else {
				VK_Play_Speech_Line(kActorBulletBob, 1350, 0.5f); //14-1350.AUD	I'd beat the crap out of him.
			}
		} else {
	...
	case 7465:                          // Low 14
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorBulletBob, 1200, 0.5f); //14-1200.AUD	Thirty five.	
				if (Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) < 50) {	
					VK_Eye_Animates(2);	
	...
	case 7475:                          // Medium 01
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorBulletBob, 1550, 0.5f); //14-1550.AUD	I'd kick the crap out of that sicko.
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 1250, 0.5f);
		}
	...
	case 7480:                          // Medium 02
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1640, 0.5f); //14-1640.AUD	Scary.
			} else {
	...
	case 7495:                          // Medium 05
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1320, 0.5f); //14-1320.AUD	Good for him.			
			} else {
				VK_Play_Speech_Line(kActorBulletBob, 1430, 0.5f); //14-1430.AUD	Man, that takes me back.
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 1320, 0.5f); //14-1320.AUD	Good for him.
		}
	...
	case 7585:                          // Medium 13
		...
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				...
				VK_Play_Speech_Line(kActorBulletBob, 1580, 0.5f); //14-1580.AUD	I'd burn the book.
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 1520, 0.5f);
		}
	...
	case 7595:                          // Medium 14
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobHamsterTalk)) {
				VK_Play_Speech_Line(kActorBulletBob, 1530, 0.5f); //14-1530.AUD	I want a hamster. Ain't you been paying attention?
			} else {
				VK_Play_Speech_Line(kActorBulletBob, 1530, 0.5f); //14-1030.AUD	I want a hamster.
			}
		} else {
			VK_Play_Speech_Line(kActorBulletBob, 1530, 0.5f); //14-1530.AUD	I want a hamster. Ain't you been paying attention?
		}
	...
	case 7600:                          // Medium 15
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1540, 0.5f); //14-1540.AUD	I'd beat some sense into him, and then toss the collection.
			} else {
				VK_Play_Speech_Line(kActorBulletBob, 1640, 0.5f); //14-1640.AUD	Scary.
			}
		} else {
	...
	case 7635:                          // High 03
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) < 50
			|| Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1590, 0.5f); //14-1590.AUD	So this is how this test goes, huh? You asking me sick questions?
		...
		VK_Play_Speech_Line(kActorBulletBob, 1600, 0.5f); //14-1600.AUD	Is that right?
		VK_Eye_Animates(3);
		if (_vm->_cutContent) {
			if (Actor_Query_Friendliness_To_Other(kActorBulletBob, kActorMcCoy) < 50
			|| Game_Flag_Query(kFlagBulletBobIsReplicant)) {
	...
	case 7770:                          // High 10
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagBulletBobIsReplicant)) {
				VK_Play_Speech_Line(kActorBulletBob, 1790, 0.5f); //14-1790.AUD	I heard worse in the wars. Hell, I've done worse.
				if (Random_Query(0, 1) == 1) {
					VK_Eye_Animates(1);
				}
			} else {
