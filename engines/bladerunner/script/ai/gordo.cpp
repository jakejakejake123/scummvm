	...
	if (_vm->_cutContent) {
		if (Actor_Query_In_Set(kActorGordo, kSetKP07)) {
			if (Actor_Query_Goal_Number(kActorGordo) < kGoalGordoGone) {
				Loop_Actor_Walk_To_Actor(kActorMcCoy, kActorGordo, 24, false, false);
				Actor_Face_Actor(kActorMcCoy, kActorGordo, true);
				Actor_Face_Actor(kActorGordo, kActorMcCoy, true);
...
void AIScriptGordo::Retired(int byActorId) {
	...
	&& Actor_Query_In_Set(kActorGordo, kSetNR01)) {
		Player_Loses_Control();
...
bool AIScriptGordo::GoalChanged(int currentGoalNumber, int newGoalNumber) {
	...
	case kGoalGordoNR02TalkAboutMcCoy:
		...
		Actor_Says(kActorGordo, 720, 16); //02-0720.AUD	I see we got a celebrity in the audience! Can I get a spot, please?
		Actor_Says(kActorGordo, 730, 18); //02-0730.AUD	That’s right. It’s one of the boys from the local Blade Runner unit!
		Music_Stop(1u);
		Actor_Face_Actor(kActorMcCoy, kActorGordo, true);
		if (_vm->_cutContent) {
			if (!Game_Flag_Query(kFlagGordoIsReplicant)
			&& !Game_Flag_Query(kFlagSadikIsReplicant)) {
				if (Actor_Query_Friendliness_To_Other(kActorGordo, kActorMcCoy) > 50) {
					...
					Actor_Says(kActorGordo, 760, 15); //02-0760.AUD	He’s protecting us poor slobs, ladies and gentlemen.
					Sound_Play(kSfxAUDLAFF1, 50, 0, 0, 50);
					Actor_Says(kActorGordo, 780, 13); //02-0780.AUD	Nice job, eh detective?
				}
			} else if (!Game_Flag_Query(kFlagGordoIsReplicant)
			&& Game_Flag_Query(kFlagSadikIsReplicant)) {
				if (Actor_Query_Friendliness_To_Other(kActorGordo, kActorMcCoy) > 50) {
		...
		if (Game_Flag_Query(kFlagGordoIsReplicant)) {
			...
			if (_vm->_cutContent) {
				...
				&& Global_Variable_Query(kVariableHollowayArrest) != 2) {
					if (Actor_Clue_Query(kActorMcCoy, kClueStolenCheese)  
					|| Actor_Clue_Query(kActorMcCoy, kClueGordoInterview3)
					|| Actor_Clue_Query(kActorMcCoy, kClueGordosLighterReplicant)
					...
					if (Actor_Query_Intelligence(kActorGordo) == 60) {
						...
						if (Player_Query_Agenda() != kPlayerAgendaPolite) {
							...
							if (Game_Flag_Query(kFlagZhoraNewsReport)) {
								if (!Game_Flag_Query(kFlagMcCoyIsHelpingReplicants)) {
									Actor_Says(kActorMcCoy, 3895, 16); //00-3895.AUD	Too bad she’ll never read it.
								}
							}
						} else {
							Actor_Says(kActorMcCoy, 8095, 13); //00-8095.AUD	That's not--
						}
					} 
		...
		if (_vm->_cutContent) {
			...
			|| Actor_Clue_Query(kActorMcCoy, kClueLichenDogWrapper)) {
				if (!Game_Flag_Query(kFlagGordoIsReplicant)) {
					...
					Player_Set_Combat_Mode(true);
					Actor_Face_Actor(kActorMcCoy, kActorGordo, true);
					Actor_Says(kActorMcCoy, 8945, -1); //00-8945.AUD	Freeze!
					Actor_Change_Animation_Mode(kActorGordo, 4);
					Delay(1000);
...
void AIScriptGordo::talkToMcCoyInCity() {
	...
	if (!Game_Flag_Query(kFlagGordoTalk1)
		...
		if (_vm->_cutContent) {
			if (Player_Query_Agenda() == kPlayerAgendaPolite) {
				Actor_Says(kActorMcCoy, 6460, 12); //00-6460.AUD	Hey, pal.
			} else {
				Actor_Says(kActorMcCoy, 3970, 14); //00-3970.AUD	Hey.
			}
		} else {
		...
		AI_Movement_Track_Pause(kActorGordo);
		if (_vm->_cutContent) {
			Actor_Says(kActorMcCoy, 8915, 12); //00-8915.AUD	You got a minute, pal?
		} else {
			Actor_Says(kActorMcCoy, 6485, 12);
		}
			...
			if (Game_Flag_Query(kFlagGordoIsReplicant)) {
				...
				if (Player_Query_Agenda() == kPlayerAgendaPolite) {
					if (Global_Variable_Query(kVariableChinyen) >= 10) {
						...
						if (Actor_Clue_Query(kActorMcCoy, kClueGordoInterview3)) {
...
void AIScriptGordo::talkToMcCoyAtNR02() {
	...
	Actor_Face_Actor(kActorGordo, kActorMcCoy, true);
	Actor_Face_Actor(kActorMcCoy, kActorGordo, true);
	Actor_Says(kActorGordo, 290, 13); //02-0290.AUD	You’re that cat from Howie Lee’s. McCoy, right? We keep bumping into each other.
...
void AIScriptGordo::dialogue2() {
	...
	if (answer == 820) { // ARREST
		...
		Actor_Says(kActorGordo, 240, 14); //02-0240.AUD	You’re not even going to arrest me?
		...
		if (Actor_Clue_Query(kActorMcCoy, kClueStolenCheese)) {
			...
			Actor_Says(kActorMcCoy, 7815, 16); //00-7815.AUD	No.
			Delay(1000);
			Actor_Says(kActorGordo, 570, 13); //02-0570.AUD	Gotta go, daddy-o.
		}
		Game_Flag_Set(kFlagGordoRanAway);
		Scene_Exits_Enable();
		Actor_Set_Goal_Number(kActorGordo, kGoalGordoNR01RunAway);
...
void AIScriptGordo::dialogue1() {
	...
	if (_vm->_cutContent) {
		if (Actor_Clue_Query(kActorMcCoy, kClueStolenCheese)) {
			DM_Add_To_List_Never_Repeat_Once_Selected(800, 9, 7, 8); // CHEESE
		}
	} else {
		...
		if (_vm->_cutContent) {
			...
			|| Actor_Query_Friendliness_To_Other(kActorGordo, kActorMcCoy) < 51) {
				...
				|| (Player_Query_Agenda() == kPlayerAgendaErratic)) {
					...
					Actor_Says(kActorGordo, 400, 12); //02-0400.AUD	What do you mean? Taffy’s got some real celebs in the audience, baby.
					Actor_Modify_Friendliness_To_Other(kActorGordo, kActorMcCoy, -2);
				}
			} else {
				Delay(1000);
	...
	case 770: // JOB
		...
		if (_vm->_cutContent) {
			if (Game_Flag_Query(kFlagZhoraNewsReport)) {
				...
				Actor_Says(kActorGordo, 420, 14); //02-0420.AUD	She went on permanent vacation.
				Actor_Says(kActorMcCoy, 3275, 15); //00-3275.AUD	That’s right. She was retired by a Blade Runner.
				if (Game_Flag_Query(kFlagGordoIsReplicant)) {
					Actor_Says(kActorGordo, 1320, 12); //02-1320.AUD	Pretty astute, my man.
				} else {
					Actor_Says(kActorGordo, 430, 12); //02-0430.AUD	She was one of those skin-jobs?
					Actor_Says(kActorGordo, 440, 13); //02-0440.AUD	Between you and me I hear they are all over the place down here.
				}
				if (!Actor_Clue_Query(kActorMcCoy, kClueGordoInterview3)
	...
	case 780: // BLOND WOMAN
		if (_vm->_cutContent) {
			Actor_Says(kActorMcCoy, 3240, 23);
		} else {
		...
		Actor_Says(kActorGordo, 500, 13); //02-0500.AUD	I might have spent an evening with her. But how’d I remember?
		if (_vm->_cutContent) {
			if (Player_Query_Agenda() == kPlayerAgendaSurly
			|| (Player_Query_Agenda() == kPlayerAgendaErratic)) {
		...
		Actor_Says(kActorGordo, 510, 15); //02-0510.AUD	The babes dig a man with a sense of humor.
#if BLADERUNNER_ORIGINAL_BUGS
		...
		if (_vm->_cutContent) {
			if (Player_Query_Agenda() == kPlayerAgendaPolite) {
				...
				Actor_Says_With_Pause(kActorMcCoy, 3295, 0.0f, 14); //00-3295.AUD	And a sense of taste. So, in answer to my question--
			}
		} else {
			Actor_Says_With_Pause(kActorMcCoy, 3295, 0.0f, 14);
		}
#endif // BLADERUNNER_ORIGINAL_BUGS
		Actor_Says(kActorGordo, 520, 12); //02-0520.AUD	I ain’t never laid eyes on that chic. But if I do, I’ll make sure to keep her all for myself.
		Delay(1000);
		if (_vm->_cutContent) {
			if (Actor_Clue_Query(kActorMcCoy, kClueStolenCheese)  
			|| Actor_Clue_Query(kActorMcCoy, kClueGordoInterview3)
	...
	case 800: // CHEESE
		...
		if (_vm->_cutContent) {
			if (Actor_Query_Intelligence(kActorGordo) == 60) { 
				Actor_Says(kActorGordo, 540, 13); //02-0540.AUD	So you’re wasting shoplifters instead of Replicants these days?
				...
				Actor_Says(kActorGordo, 560, 13);
				if (Player_Query_Agenda() != kPlayerAgendaPolite) {
					Actor_Says(kActorMcCoy, 3310, kAnimationModeTalk); //00-3310.AUD	You know a girl named Lucy, Gordo? You steal that cheese for her maybe?
					Delay(1000);
					Actor_Says(kActorMcCoy, 3315, 15);
					Actor_Says(kActorGordo, 570, 14);
					Game_Flag_Reset(kFlagMcCoyIsHelpingReplicants);
