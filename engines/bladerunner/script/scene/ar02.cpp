	...
	if (_vm->_cutContent) {
		if (actorId == kActorHasan
			...
			if (!Loop_Actor_Walk_To_XYZ(kActorMcCoy, -240.79f, 0.0f, -1328.89f, 12, true, false, false)) {
				...
				&& !Actor_Clue_Query(kActorMcCoy, kCluePeruvianLadyInterview)) {
				...
				&& Actor_Clue_Query(kActorMcCoy, kClueBobRobbed)
...
void SceneScriptAR02::dialogueWithInsectDealerBuyBracelet() {
	...
	if (_vm->_cutContent) {
		...
		|| Query_Difficulty_Level() == kGameDifficultyEasy) {
			if (Player_Query_Agenda() == kPlayerAgendaSurly 
			|| Player_Query_Agenda() == kPlayerAgendaErratic) {
				DM_Add_To_List_Never_Repeat_Once_Selected(540, 1, 2, 7); // NO THANKS				
			} else {
	...
	if (answerValue == 530) { // BUY	
		if (_vm->_cutContent) {
			...
			Actor_Change_Animation_Mode(kActorMcCoy, 23);
			Actor_Change_Animation_Mode(kActorInsectDealer, 23);
			Delay(800);
...
void SceneScriptAR02::dialogueWithInsectDealerBuySlug() {
	...
	if (answerValue == 530) { // BUY
		...
		if (_vm->_cutContent) {
			...
			Actor_Change_Animation_Mode(kActorInsectDealer, 23);
			Delay(800);
			Item_Pickup_Spin_Effect_From_Actor(kModelAnimationSlug, kActorMcCoy, 0, 0);
			Delay(800);
		}
		if (Query_Difficulty_Level() != kGameDifficultyEasy) {
...
void SceneScriptAR02::dialogueWithHassan() {
	...
	if (_vm->_cutContent) {
		if (Global_Variable_Query(kVariableChapter) < 4) {
			if (!Game_Flag_Query(kFlagWrongInvestigation)
			&& (Actor_Clue_Query(kActorMcCoy, kClueStrangeScale1)
			|| Actor_Clue_Query(kActorMcCoy, kClueStrangeScale2))) {
	...
	if (_vm->_cutContent) {
		if (Global_Variable_Query(kVariableChapter) < 4) {
			if (Actor_Clue_Query(kActorMcCoy, kClueBobRobbed)) {
				...
				&& !Game_Flag_Query(kFlagIzoOnTheRun)
				&& Game_Flag_Query(kFlagDNARowAvailableTalk)) {
					DM_Add_To_List(1250, -1, -1, 1); // ARREST
				}
			}
		}
	}
...
void SceneScriptAR02::dialogueWithHassanBuySnake() {
	...
	if (answerValue == 530) { // BUY
		if (_vm->_cutContent) {
			Actor_Says(kActorMcCoy, 7000, 13);
		} else {
			Actor_Says(kActorMcCoy, 7000, 23);
		}
		if (_vm->_cutContent) {
			Actor_Change_Animation_Mode(kActorMcCoy, 23);
			Actor_Change_Animation_Mode(kActorHasan, 23);
			Delay(800);
			Item_Pickup_Spin_Effect_From_Actor(kModelAnimationGarterSnake, kActorMcCoy, 0, 0);
			Delay(800);
		}
		if (Query_Difficulty_Level() != kGameDifficultyEasy) {
